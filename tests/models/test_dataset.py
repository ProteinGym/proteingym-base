...
def test_dataset_empty_sequences(name, description, version, sequences):
    ds = Dataset(
        name=name, description=description, version=version, sequences=sequences
    )
    assert ds.name == name
...
@pytest.mark.parametrize(
    ...
    [
        (
            "test_dataset",
            "A test dataset which contains sequences.",
            "1.0",
            [
                {
                    "name": "seq1",
...
def test_dataset_dump(name, description, version, sequences):
    ...
    assert Path("test_dir/sequences/seq1.fasta").exists()
