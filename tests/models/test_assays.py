...
from pg2_dataset.models.assays import AssayCondition, AssayManifestSection, Assay, AssayDataType
    ...
    with pytest.raises(ValueError):
        Assay.from_manifest_section(
            AssayManifestSection(
                ...
                target="target",
                path=assay_file,
                conditions={"bad_condition": "true",}
            ),
            conditions
        )


def test_as_manifest_section(tmp_path):
    assay = Assay(
        records=[("ARFS", 1), ("SDFSDf", 2)],
    )
    manifest = assay.as_manifest_section(path=tmp_path)
    assert manifest.path == tmp_path / "assay.csv"
