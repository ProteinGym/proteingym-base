...
from pg2_dataset.primitives.constants import ENGINEERING_ROUND, SEQUENCE, SPLIT
from pg2_dataset.primitives.record import Record
from pg2_dataset.primitives.split import SPLIT_STRATEGY_MAPPING
...
class RecordsDataset(AbstractDataset):
    ...
    _split_strategy: AbstractSplitStrategy = PrivateAttr(default=None)
    _split_strategy_name: str = PrivateAttr(default=None)
    ...
    def _get_split(self, split_name: TrainTestValid) -> pd.DataFrame:
        if self._split_strategy:
            return self._internal_data_frame.filter(
                pl.col(self._split_strategy_name) == split_name
            ).to_pandas()
    ...
    def _from_csv(self) -> pl.DataFrame:
        ...
        if self.meta.split_strategy_enum:
            self._split_strategy = SPLIT_STRATEGY_MAPPING[self.meta.split_strategy_enum](**self.meta.split_strategy_kwargs)
            
            self._split_strategy_name = self._split_strategy.__class__.__name__

            split_map = self._split_strategy.split(data.to_pandas())

            data = data.with_columns(
                pl.col(SEQUENCE).replace_strict(split_map).alias(self._split_strategy_name)
            )
